...
export default function IndicadoresPvpsAlocacaoPage({ isOnline, profile }: IndicadoresPvpsAlocacaoPageProps) {
  ...
  const activeMetrics = useMemo(() => {
    ...
    const dayScopedErrors = loadingDetails
      ? (selectedDaySeries?.nao_conformes ?? 0)
      : dayDetails.filter((row) => row.status_dashboard === "nao_conforme").length;
  ...
  }, [dayDetails, loadingDetails, selectedDay, selectedDaySeries, selectedMonthLabel, showingMonthDetails, summary]);
            ...
            <section className="indicadores-panel indicadores-panel-wide indicadores-panel-conferencia indicadores-panel-conferencia-pvps-aloc">
              <div className="indicadores-panel-head">
                ...
                <button
                  ...
                  title="Ver Conformidade do mês em tela cheia"
                  aria-label="Ver Conformidade do mês em tela cheia"
                >
                  {expandIcon()}
                </button>
              </div>
            ...
            <section className="indicadores-panel indicadores-panel-wide indicadores-panel-zonas">
              <div className="indicadores-panel-head">
                ...
                <button
                  ...
                  title="Ver Erro por zona em tela cheia"
                  aria-label="Ver Erro por zona em tela cheia"
                >
                  {expandIcon()}
                </button>
              </div>
      ...
      {fullscreenPanel && typeof document !== "undefined"
        ? createPortal(
            <div
              ...
              role="dialog"
              aria-modal="true"
              aria-labelledby="indicadores-pvps-aloc-fullscreen-title"
            ...
            >
              <div className="indicadores-fullscreen-dialog surface-enter" onClick={(event) => event.stopPropagation()}>
                <div className="indicadores-fullscreen-head">
                  <div>
                    <h3 id="indicadores-pvps-aloc-fullscreen-title">
                      {fullscreenPanel === "daily" ? "Conformidade do mês" : "Erro por zona"}
                    </h3>
                    <span>
                      {fullscreenPanel === "daily"
                        ? loadingDashboard
                          ? "Atualizando..."
                          : `Barras de ${eligibleAuditLabel(selectedType).toLocaleLowerCase("pt-BR")} e linha de conformidade`
                        : "Exibe somente não conformes por zona."}
                    </span>
                  </div>
                  <button
                    type="button"
