...
import {
  ...
  saveVolumeAvulsoPreferences,
  removeLocalVolume
} from "./storage";
import {
  cancelVolume,
  countVolumeAvulsoReportRows,
  fetchCdOptions,
  fetchActiveVolume,
  fetchManifestBundle,
  fetchManifestMeta,
  fetchManifestVolumes,
  fetchVolumeAvulsoReportRows,
  fetchRouteOverview,
  fetchVolumeItems,
  finalizeVolume,
  normalizeBarcode,
...
interface ConferenciaVolumeAvulsoPageProps {
  isOnline: boolean;
...
const MODULE_DEF = getModuleByKeyOrThrow("conferencia-volume-avulso");
...
function fixedCdFromProfile(profile: VolumeAvulsoModuleProfile): number | null {
...
function buildManifestInfoLine(params: {
  ...
  if (params.hasVolumeAvulsoBase) {
    return `Base local: Volume Avulso ${params.termoRows} item(ns) | Barras ${params.barrasRows} item(ns)${updatedText}`;
  }

  return `Sem base local do Volume Avulso. Barras local: ${params.barrasRows} item(ns)${updatedText}`;
}

function withDivergencia(item: VolumeAvulsoLocalItem): {
  item: VolumeAvulsoLocalItem;
  divergencia: VolumeAvulsoDivergenciaTipo;
  qtd_falta: number;
  qtd_sobra: number;
} {
  ...
  const divergencia: VolumeAvulsoDivergenciaTipo = qtdFalta > 0 ? "falta" : qtdSobra > 0 ? "sobra" : "correto";
  return { item, divergencia, qtd_falta: qtdFalta, qtd_sobra: qtdSobra };
}

function itemSort(a: VolumeAvulsoLocalItem, b: VolumeAvulsoLocalItem): number {
...
function createLocalVolumeFromRemote(
  ...
  items: VolumeAvulsoItemRow[]
): VolumeAvulsoLocalVolume {
  const confDate = volume.conf_date || todayIsoBrasilia();
  const localKey = buildVolumeAvulsoVolumeKey(profile.user_id, volume.cd, confDate, volume.nr_volume);
  const localItems: VolumeAvulsoLocalItem[] = items.map((item) => ({
    coddv: item.coddv,
...
function createLocalVolumeFromManifest(
  ...
  cd: number,
  idEtiqueta: string,
  manifestItems: VolumeAvulsoManifestItemRow[]
): VolumeAvulsoLocalVolume {
  const nowIso = new Date().toISOString();
  const confDate = todayIsoBrasilia();
  const first = manifestItems[0];
  const localKey = buildVolumeAvulsoVolumeKey(profile.user_id, cd, confDate, idEtiqueta);
  const items: VolumeAvulsoLocalItem[] = manifestItems.map((row) => ({
    coddv: row.coddv,
    barras: null,
...
function normalizeRpcErrorMessage(value: string): string {
  ...
  if (value.includes("CONFERENCIA_EM_ABERTO_OUTRO_VOLUME")) {
    return "Já existe uma conferência em andamento para sua matrícula. Finalize o NR Volume atual para iniciar outro.";
  }
...
function conferenceActionLabel(status: VolumeAvulsoStoreStatus): string {
  return status === "pendente" ? "Iniciar conferência" : "Abrir leitura";
}
...
function buildConferenceReportKey(
  value: Pick<VolumeAvulsoReportRow, "conf_date" | "cd" | "nr_volume">
): string {
  return `${value.conf_date}|${value.cd}|${value.nr_volume}`;
}
...
export default function ConferenciaVolumeAvulsoPage({ isOnline, profile }: ConferenciaVolumeAvulsoPageProps) {
  ...
  const [reportCount, setReportCount] = useState<VolumeAvulsoReportCount | null>(null);
  ...
  const validateReportFilters = useCallback((): VolumeAvulsoReportFilters | null => {
  ...
  const runReportSearch = useCallback(async () => {
    ...
    try {
      const count = await countVolumeAvulsoReportRows(filters);
  ...
  const runReportExport = useCallback(async () => {
    ...
    try {
      ...
      const itemRows: VolumeAvulsoReportRow[] = [];
      let offset = 0;

      while (offset < expectedItems) {
        const batch = await fetchVolumeAvulsoReportRows(filters, offset, REPORT_PAGE_SIZE);
      ...
      const summarySheet = XLSX.utils.aoa_to_sheet([
        ["Relatório de Conferência de Volume Avulso"],
      ...
      itemsSheet["!cols"] = [
        ...
        { wch: 18 }, { wch: 18 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 18 },
        { wch: 24 }, { wch: 22 }
      ];
      ...
      XLSX.writeFile(
        workbook,
        `relatorio-conferencia-volume-avulso-${filters.dtIni}-${filters.dtFim}-cd${String(filters.cd).padStart(2, "0")}.xlsx`,
        { compression: true }
      );
  ...
  const persistPreferences = useCallback(async (next: {
    ...
    const current = await getVolumeAvulsoPreferences(profile.user_id);
    await saveVolumeAvulsoPreferences(profile.user_id, {
  ...
  const runPendingSync = useCallback(async (silent = false) => {
    ...
    try {
      const result = await syncPendingVolumeAvulsoVolumes(profile.user_id);
    ...
    try {
      ...
      if (!isOnline) {
        if (!localMeta || localMeta.row_count <= 0) {
          throw new Error("Sem base local de Volume Avulso. Conecte-se e sincronize antes de usar offline.");
        }
      ...
      if (shouldDownload) {
        const bundle = await fetchManifestBundle(currentCd, (progress) => {
          if (progress.step === "items") {
            if (progress.total > 0) {
              setProgressMessage(
                `Atualizando base de Volume Avulso... ${progress.percent}% (${progress.rows}/${progress.total})`
              );
              return;
            }
            setProgressMessage(`Atualizando base de Volume Avulso... ${progress.percent}%`);
            return;
          }
      ...
      setManifestInfo(
        buildManifestInfoLine({
          ...
          hasVolumeAvulsoBase: (metaAfterSync?.row_count ?? termoRowCount) > 0
        })
      );

      setManifestReady(true);
      if (!background) {
        setStatusMessage("Base de Volume Avulso pronta para trabalho offline.");
      }
    } finally {
  ...
  const applyVolumeUpdate = useCallback(async (nextVolume: VolumeAvulsoLocalVolume, focusInput = true) => {
  ...
  const resumeRemoteActiveVolume = useCallback(async (silent = false): Promise<VolumeAvulsoLocalVolume | null> => {
    ...
    if (!silent) {
      setStatusMessage(`Conferência retomada automaticamente: NR Volume ${localVolume.nr_volume}.`);
    }
  ...
  const openVolumeFromEtiqueta = useCallback(async (rawEtiqueta: string) => {
    ...
    if (!etiqueta) {
      setErrorMessage("Informe o NR Volume para abrir o volume.");
      return;
    }
    if (currentCd == null) {
    ...
    if (hasOpenConference && activeVolume && activeVolume.nr_volume !== etiqueta) {
      setErrorMessage("Existe uma conferência em andamento para sua matrícula. Finalize o NR Volume atual para iniciar outro.");
      setStatusMessage(`Conferência ativa: NR Volume ${activeVolume.nr_volume}.`);
    ...
    try {
      ...
      if (preferOfflineMode) {
        ...
        if (!manifestItems.length) {
          showDialog({
            title: "NR Volume inválido",
            message: "NR Volume não encontrado na base local de Volume Avulso para este CD."
          });
          return;
        }
      ...
      if (message.includes("CONFERENCIA_EM_ABERTO_OUTRO_VOLUME")) {
        try {
          ...
          if (resumed) {
            ...
            setErrorMessage(null);
            setStatusMessage(`Conferência retomada automaticamente: NR Volume ${resumed.nr_volume}.`);
            return;
          }
        } catch {
  ...
  useEffect(() => {
    ...
    const loadInitial = async () => {
      try {
        await cleanupExpiredVolumeAvulsoVolumes(profile.user_id);
        const prefs = await getVolumeAvulsoPreferences(profile.user_id);
        if (cancelled) return;
        ...
        if (cancelled) return;
        const message = error instanceof Error ? error.message : "Falha ao carregar módulo Volume Avulso.";
        setErrorMessage(normalizeRpcErrorMessage(message));
      }
    };
    ...
    const loadLocalContext = async () => {
      ...
      setManifestInfo(
        buildManifestInfoLine({
          ...
          hasVolumeAvulsoBase: Boolean(localMeta && localMeta.row_count > 0)
        })
      );
      setRouteRows(localRoutes);
      setManifestVolumeRows(localManifestVolumes);
  ...
  const handleToggleOffline = async () => {
    ...
    if (next) {
      if (isOnline) {
        ...
        setErrorMessage("Sem base local do Volume Avulso. Conecte-se para sincronizar a base offline.");
      }
      return;
    }
    ...
    <>
      ...
      <section className="modules-shell termo-shell">
        <div className="termo-head">
          ...
          <p>Para trabalhar offline, sincronize a base de Volume Avulso.</p>
        ...
        {showReportPanel && canSeeReportTools ? (
          <section className="termo-report-panel">
            <div className="termo-report-head">
              <h3>Relatório de Conferência de Volume Avulso</h3>
            ...
            <label>
              ...
              <div className="input-icon-wrap with-action">
                ...
                <input
                  ...
                  autoCorrect="off"
                  spellCheck={false}
                  placeholder="Informe o NR Volume"
                  required
                />
                <button
                  type="button"
                  ...
                  title="Ler NR Volume pela câmera"
                  aria-label="Ler NR Volume pela câmera"
        ...
        {activeVolume ? (
          ...
          <div className="coleta-empty">
            Nenhum volume ativo. Informe um NR Volume para iniciar a conferência.
          </div>
        )}
      </section>
              ...
              <div className="scanner-dialog surface-enter" onClick={(event) => event.stopPropagation()}>
                <div className="scanner-head">
                  <h3 id="termo-scanner-title">
                    {scannerTarget === "etiqueta" ? "Scanner de NR Volume" : "Scanner de barras"}
                  </h3>
