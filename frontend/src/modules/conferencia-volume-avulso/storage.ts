...
const DB_NAME = "auditoria-volume-avulso-v1";
const DB_VERSION = 1;
...
interface ManifestItemStoreRow extends VolumeAvulsoManifestItemRow {
  key: string;
  user_id: string;
  cd: number;
}

interface ManifestBarrasStoreRow extends VolumeAvulsoManifestBarrasRow {
  key: string;
  user_id: string;
  cd: number;
}

interface ManifestMetaStoreRow extends VolumeAvulsoManifestMeta {
  key: string;
  user_id: string;
  cached_at: string;
}
...
interface RouteOverviewStoreRow {
  ...
  user_id: string;
  cd: number;
  rows: VolumeAvulsoRouteOverviewRow[];
  updated_at: string;
}

interface PrefStoreRow {
  key: string;
  value: VolumeAvulsoPreferences;
}
...
function normalizePrefs(value: Partial<VolumeAvulsoPreferences> | null | undefined): VolumeAvulsoPreferences {
  return {
...
export function buildVolumeAvulsoVolumeKey(userId: string, cd: number, confDate: string, idEtiqueta: string): string {
...
function sortVolumes(rows: VolumeAvulsoLocalVolume[]): VolumeAvulsoLocalVolume[] {
...
export async function saveVolumeAvulsoPreferences(userId: string, value: VolumeAvulsoPreferences): Promise<void> {
  const db = await getDb();
...
export async function listManifestItemsByCd(
  ...
  return rows
    ...
    .sort((a, b) => {
      const byVolume = a.nr_volume.localeCompare(b.nr_volume, "pt-BR", { numeric: true, sensitivity: "base" });
...
export async function findManifestBarras(
  ...
  cd: number,
  barras: string
): Promise<VolumeAvulsoManifestBarrasRow | null> {
...
export async function getRouteOverviewLocal(userId: string, cd: number): Promise<VolumeAvulsoRouteOverviewRow[]> {
  const db = await getDb();
...
export async function saveLocalVolume(volume: VolumeAvulsoLocalVolume): Promise<void> {
  const db = await getDb();
...
export async function getLocalVolume(
  ...
  confDate: string,
  idEtiqueta: string
): Promise<VolumeAvulsoLocalVolume | null> {
  const db = await getDb();
...
export async function getPendingSummary(userId: string): Promise<VolumeAvulsoPendingSummary> {
...
export async function cleanupExpiredVolumeAvulsoVolumes(userId: string): Promise<number> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      const row = cursor.value as VolumeAvulsoLocalVolume;
...
export async function clearUserVolumeAvulsoSessionCache(userId: string): Promise<void> {
