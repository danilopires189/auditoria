import type {
  ControleAvariasPreferences,
  ControleAvariasRow,
  ControleAvariasSyncStatus,
  DbBarrasCacheRow,
  DbBarrasSyncMeta
} from "./types";

const DB_NAME = "auditoria-controle-avarias-v1";
const DB_VERSION = 1;

const STORE_DB_BARRAS = "db_barras";
const STORE_META = "meta";
const STORE_COLETA_ROWS = "controle_avarias_rows";
...
const PENDING_STATUSES: ControleAvariasSyncStatus[] = ["pending_insert", "pending_update", "pending_delete", "error"];
...
export async function upsertControleAvariasRow(row: ControleAvariasRow): Promise<void> {
  const db = await getDb();
...
export async function removeControleAvariasRow(localId: string): Promise<void> {
  const db = await getDb();
  const transaction = db.transaction(STORE_COLETA_ROWS, "readwrite");
...
export async function getUserControleAvariasRows(userId: string): Promise<ControleAvariasRow[]> {
  const db = await getDb();
  ...
  const rows = (await requestToPromise(index.getAll(userId))) as ControleAvariasRow[];
  await transactionDone(transaction);
  return sortRows(rows ?? []);
}

export async function getPendingRows(userId: string): Promise<ControleAvariasRow[]> {
  const db = await getDb();
  const transaction = db.transaction(STORE_COLETA_ROWS, "readonly");
  ...
  const rows = batches.flatMap((batch) => (Array.isArray(batch) ? (batch as ControleAvariasRow[]) : []));
  return sortRows(rows);
}
...
export async function getControleAvariasPreferences(userId: string): Promise<ControleAvariasPreferences> {
  const db = await getDb();
...
export async function saveControleAvariasPreferences(userId: string, preferences: ControleAvariasPreferences): Promise<void> {
  const db = await getDb();
...
export async function cleanupExpiredControleAvariasRows(userId: string, ttlMs = ONE_DAY_MS): Promise<number> {
  const db = await getDb();
  const transaction = db.transaction(STORE_COLETA_ROWS, "readwrite");
  ...
  await new Promise<void>((resolve, reject) => {
    request.onerror = () => reject(request.error ?? new Error("Falha ao limpar avarias expiradas."));
    request.onsuccess = () => {
      ...
      const row = cursor.value as ControleAvariasRow;
...
export async function clearUserControleAvariasSessionCache(userId: string): Promise<void> {
  const db = await getDb();
  ...
  await new Promise<void>((resolve, reject) => {
    request.onerror = () => reject(request.error ?? new Error("Falha ao limpar sessão de controle de avarias."));
    request.onsuccess = () => {
      const cursor = request.result;
      if (!cursor) {
        resolve();
