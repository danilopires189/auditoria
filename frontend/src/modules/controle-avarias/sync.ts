...
function parseOrigem(value: unknown): ControleAvariasOrigem {
  return value === "Expedição" || value === "Pulmão" || value === "Separação"
    ? value
    : "Expedição";
}
...
function mapRpcRowToReport(raw: Record<string, unknown>): ControleAvariasReportRow {
  return {
    id: String(raw.id ?? ""),
    etiqueta: parseNullableString(raw.etiqueta),
    cd: parseInteger(raw.cd),
...
export async function fetchTodaySharedControleAvariasRows(cd: number, limit = 1200): Promise<ControleAvariasRow[]> {
  if (!supabase) throw new Error("Supabase não inicializado.");

  const { data, error } = await supabase.rpc("rpc_controle_avarias_today", {
    p_cd: cd,
    p_limit: limit
  });

  if (error) {
    throw new Error(`Falha ao buscar avarias do dia: ${toErrorMessage(error)}`);
  }

  if (!Array.isArray(data)) return [];

  return data
    .map((item) => mapRpcRowToControleAvariasRow(item as Record<string, unknown>))
    .filter((item) => item.remote_id != null);
}

function patchError(row: ControleAvariasRow, message: string): ControleAvariasRow {
  return {
    ...row,
    sync_status: "error",
    sync_error: message,
...
async function syncInsert(row: ControleAvariasRow): Promise<void> {
  if (!supabase) throw new Error("Supabase não inicializado.");

  const { data, error } = await supabase.rpc("rpc_controle_avarias_insert", {
    p_cd: row.cd,
    p_barras: row.barras,
    p_qtd: row.qtd,
  ...
  if (!first || typeof first !== "object") {
    throw new Error("Resposta inválida ao inserir avaria.");
  }
}

async function syncUpdate(row: ControleAvariasRow): Promise<void> {
  if (!supabase) throw new Error("Supabase não inicializado.");
  if (!row.remote_id) {
    throw new Error("Linha sem ID remoto para atualização.");
  }

  const { data, error } = await supabase.rpc("rpc_controle_avarias_update", {
    p_id: row.remote_id,
    p_qtd: row.qtd,
    p_etiqueta: row.etiqueta,
    p_motivo: row.motivo,
  ...
  const first = Array.isArray(data) ? data[0] : null;
  if (!first || typeof first !== "object") {
    throw new Error("Resposta inválida ao atualizar avaria.");
  }
}

async function syncDelete(row: ControleAvariasRow): Promise<void> {
  if (!supabase) throw new Error("Supabase não inicializado.");
  ...
  const { data, error } = await supabase.rpc("rpc_controle_avarias_delete", {
    p_id: row.remote_id
  });
...
export async function syncPendingControleAvariasRows(userId: string): Promise<{
  processed: number;
  synced: number;
  failed: number;
  pending: number;
  ...
  for (const row of pendingRows) {
    try {
      ...
      failed += 1;
      await upsertControleAvariasRow(patchError(row, toErrorMessage(error)));
    }
  }
...
export async function countControleAvariasReportRows(filters: ControleAvariasReportFilters): Promise<number> {
  if (!supabase) throw new Error("Supabase não inicializado.");

  const { data, error } = await supabase.rpc("rpc_controle_avarias_report_count", {
...
export async function fetchControleAvariasReportRows(
  filters: ControleAvariasReportFilters,
  limit = 20000
): Promise<ControleAvariasReportRow[]> {
  if (!supabase) throw new Error("Supabase não inicializado.");

  const { data, error } = await supabase.rpc("rpc_controle_avarias_report_rows", {
    p_dt_ini: filters.dtIni,
    p_dt_fim: filters.dtFim,
    p_cd: filters.cd,
    p_limit: limit
