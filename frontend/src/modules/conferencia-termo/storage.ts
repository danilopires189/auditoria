...
interface ManifestItemStoreRow extends TermoManifestItemRow {
  key: string;
  user_id: string;
  cd: number;
}

interface ManifestBarrasStoreRow extends TermoManifestBarrasRow {
  key: string;
  user_id: string;
  cd: number;
}

interface ManifestMetaStoreRow extends TermoManifestMeta {
  key: string;
  user_id: string;
  cached_at: string;
}
...
interface RouteOverviewStoreRow {
  ...
  user_id: string;
  cd: number;
  rows: TermoRouteOverviewRow[];
  updated_at: string;
}

interface PrefStoreRow {
  key: string;
  value: TermoPreferences;
}
  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(STORE_MANIFEST_ITEMS)) {
        ...
        store.createIndex(INDEX_ITEMS_BY_USER_CD_ETIQUETA, ["user_id", "cd", "id_etiqueta"], { unique: false });
      }
      ...
      if (!db.objectStoreNames.contains(STORE_VOLUMES)) {
        ...
        store.createIndex(INDEX_VOLUMES_BY_USER_CD_ETIQUETA, ["user_id", "cd", "id_etiqueta"], { unique: false });
...
function sortVolumes(rows: TermoLocalVolume[]): TermoLocalVolume[] {
...
export async function getTermoPreferences(userId: string): Promise<TermoPreferences> {
  const db = await getDb();
...
export async function saveTermoPreferences(userId: string, value: TermoPreferences): Promise<void> {
  const db = await getDb();
...
export async function listManifestItemsByCd(
  ...
  return rows
    ...
    .sort((a, b) => {
      const byEtiqueta = a.id_etiqueta.localeCompare(b.id_etiqueta, "pt-BR", { numeric: true, sensitivity: "base" });
      if (byEtiqueta !== 0) return byEtiqueta;
...
export async function findManifestBarras(
  ...
  cd: number,
  barras: string
): Promise<TermoManifestBarrasRow | null> {
...
export async function getRouteOverviewLocal(userId: string, cd: number): Promise<TermoRouteOverviewRow[]> {
  const db = await getDb();
...
export async function saveLocalVolume(volume: TermoLocalVolume): Promise<void> {
  const db = await getDb();
...
export async function getLocalVolume(
  ...
  confDate: string,
  idEtiqueta: string
): Promise<TermoLocalVolume | null> {
  const db = await getDb();
  const transaction = db.transaction(STORE_VOLUMES, "readonly");
  const store = transaction.objectStore(STORE_VOLUMES);
  const key = buildTermoVolumeKey(userId, cd, confDate, idEtiqueta.trim());
  const raw = await requestToPromise(store.get(key));
  await transactionDone(transaction);
  return (raw as TermoLocalVolume | undefined) ?? null;
}
...
export async function listUserLocalVolumes(userId: string): Promise<TermoLocalVolume[]> {
  const db = await getDb();
  const transaction = db.transaction(STORE_VOLUMES, "readonly");
  const index = transaction.objectStore(STORE_VOLUMES).index(INDEX_VOLUMES_BY_USER);
  const rows = (await requestToPromise(index.getAll(IDBKeyRange.only(userId)))) as TermoLocalVolume[];
  await transactionDone(transaction);
...
export async function cleanupExpiredTermoVolumes(userId: string): Promise<number> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      const row = cursor.value as TermoLocalVolume;
...
export async function clearUserTermoSessionCache(userId: string): Promise<void> {
