...
function mapReportCount(raw: Record<string, unknown>): TermoReportCount {
  return {
...
function mapReportRow(raw: Record<string, unknown>): TermoReportRow {
  const statusRaw = String(raw.status ?? "em_conferencia");
  const status = statusRaw === "finalizado_ok" || statusRaw === "finalizado_falta"
    ? statusRaw
    : "em_conferencia";
...
export async function fetchManifestBundle(
  ...
  if (includeBarras) {
    ...
    while (true) {
      let page: TermoManifestBarrasRow[];
      try {
...
export async function countTermoReportRows(
  filters: TermoReportFilters
): Promise<TermoReportCount> {
  if (!supabase) throw new Error("Supabase não inicializado.");

  const { data, error } = await supabase.rpc("rpc_conf_termo_report_count", {
...
export async function fetchTermoReportRows(
  filters: TermoReportFilters,
  offset = 0,
  limit = 1000
): Promise<TermoReportRow[]> {
  if (!supabase) throw new Error("Supabase não inicializado.");

  const { data, error } = await supabase.rpc("rpc_conf_termo_report_rows", {
    p_dt_ini: filters.dtIni,
    p_dt_fim: filters.dtFim,
    p_cd: filters.cd,
    p_offset: Math.max(offset, 0),
