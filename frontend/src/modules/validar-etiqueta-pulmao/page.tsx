...
import { getValidarEtiquetaPulmaoPreferences, saveValidarEtiquetaPulmaoPreferences } from "./storage";
import {
...
import type {
  ...
  ValidarEtiquetaPulmaoModuleProfile
} from "./types";

interface ValidarEtiquetaPulmaoPageProps {
  isOnline: boolean;
  profile: ValidarEtiquetaPulmaoModuleProfile;
}

type BarcodeValidationState = "idle" | "validating" | "valid" | "invalid";
type InternalCodeValidationState = "idle" | "validating" | "valid" | "invalid";
type ScannerInputTarget = "produto" | "codigoInterno";
type PopupTone = "success" | "error";
type PopupResetMode = "full" | "barcode-only";
...
const MODULE_DEF = getModuleByKeyOrThrow("validar-etiqueta-pulmao");
...
function fixedCdFromProfile(profile: ValidarEtiquetaPulmaoModuleProfile): number | null {
...
function detectCodigoInternoInputFormatError(value: string): string | null {
  if (!value) return "Informe o código interno.";
  if (!DIGITS_PATTERN.test(value)) {
    return "Código interno inválido. Informe somente números.";
  }
  const parsed = Number.parseInt(value, 10);
  if (!Number.isFinite(parsed) || parsed <= 0) return "Código interno inválido.";
  return null;
}
...
function asLookupSummary(result: ValidarEtiquetaPulmaoLookupResult): string {
  return `${result.descricao || `CODDV ${result.coddv}`} | CODDV ${result.coddv}`;
}

function pickMainBarcode(result: ValidarEtiquetaPulmaoLookupResult): string {
...
export default function ValidarEtiquetaPulmaoPage({ isOnline, profile }: ValidarEtiquetaPulmaoPageProps) {
  ...
  const scannerInputStateRef = useRef<Record<ScannerInputTarget, ScannerInputState>>({
    ...
    codigoInterno: createScannerInputState()
  });
  ...
  const {
    inputMode: codigoInternoInputMode,
    enableSoftKeyboard: enableCodigoInternoSoftKeyboard,
    disableSoftKeyboard: disableCodigoInternoSoftKeyboard
  } = useOnDemandSoftKeyboard("numeric");

  const [produtoInput, setProdutoInput] = useState("");
  const [codigoInternoInput, setCodigoInternoInput] = useState("");
  const [codigoInternoAtual, setCodigoInternoAtual] = useState<number | null>(null);
  ...
  const [codigoInternoValidationState, setCodigoInternoValidationState] = useState<InternalCodeValidationState>("idle");
  const [currentProduct, setCurrentProduct] = useState<ValidarEtiquetaPulmaoLookupResult | null>(null);
  const [validationPopup, setValidationPopup] = useState<ValidationPopupState | null>(null);

  const [scannerOpen, setScannerOpen] = useState(false);
  const [scannerTarget, setScannerTarget] = useState<ScannerInputTarget>("codigoInterno");
  ...
  const activeInputTarget: ScannerInputTarget = codigoInternoAtual == null ? "codigoInterno" : "produto";
  const activeInputValue = activeInputTarget === "produto" ? produtoInput : codigoInternoInput;
  const activeValidationState = activeInputTarget === "produto" ? produtoValidationState : codigoInternoValidationState;
  const activeInputMode = activeInputTarget === "produto" ? produtoInputMode : codigoInternoInputMode;
  const activeFieldLabel = activeInputTarget === "produto" ? "Código de barras" : "Código interno (QR Code)";
  const activeFieldPlaceholder = activeInputTarget === "produto"
    ? "Leia o código de barras"
    : "Leia o QR Code do código interno";
  ...
  const clearValidationCycle = useCallback(() => {
    ...
    setProdutoValidationState("idle");
    setCodigoInternoValidationState("idle");
    focusProduto();
  }, [focusProduto]);
  ...
  const showValidationPopup = useCallback((params: {
    ...
    setValidationPopup({
      ...
      detailsList: params.detailsList,
      manualClose: Boolean(params.manualClose),
      resetMode: params.resetMode ?? "full"
    });
  ...
  const closeValidationPopup = useCallback(() => {
    ...
    if (validationPopup?.resetMode === "barcode-only") {
      clearBarcodeStep();
      return;
    }
    clearValidationCycle();
  }, [clearBarcodeStep, clearPopupTimer, clearValidationCycle, validationPopup?.resetMode]);

  const queueValidationAudit = useCallback((params: {
    codigoInterno: number;
    barras: string;
    product: ValidarEtiquetaPulmaoLookupResult | null;
    validado: boolean;
  }) => {
    if (!params.barras) return;

    void enqueueValidarEtiquetaPulmaoAudit({
      userId: profile.user_id,
      isOnline,
      payload: {
        cd: params.product?.cd && params.product.cd > 0 ? params.product.cd : (currentCd ?? 0),
        codigo_interno: params.codigoInterno,
        barras: params.barras,
        coddv_resolvido: params.product?.coddv ?? null,
        descricao: params.product?.descricao ?? null,
  ...
  const runOfflineSync = useCallback(async () => {
    ...
    setProgressMessage("Iniciando sincronização da base offline...");
    ...
    try {
      ...
      setProgressMessage(null);
      setErrorMessage(`Falha ao sincronizar db_barras: ${normalizeLookupError(error)}`);
    } finally {
      setBusySync(false);
    }
  }, [isOnline, refreshOfflineMeta]);
  ...
  const commitProdutoInput = useCallback(async (rawValue: string) => {
    ...
    if (codigoInternoAtual == null || codigoInternoAtual <= 0) {
      setErrorMessage("Informe o código interno antes de validar a etiqueta.");
      setProdutoValidationState("invalid");
      setCodigoInternoValidationState("invalid");
      focusProduto();
      return;
    ...
    try {
      ...
      if (resolved.coddv === codigoInternoAtual) {
        ...
        showValidationPopup({
          tone: "success",
          title: "Etiqueta validada",
          detail: `${asLookupSummary(resolved)} | Código interno ${codigoInternoAtual}`,
          detailsList: [],
      ...
      triggerScanErrorAlert("Código interno divergente.");
      showValidationPopup({
        tone: "error",
        title: "Etiqueta não corresponde",
        detail: `${asLookupSummary(resolved)} | Barra ${pickMainBarcode(resolved) || value}`,
        detailsList: [
          `Código interno informado: ${codigoInternoAtual}`,
          `Código interno correto: ${resolved.coddv}`
        ],
        manualClose: true,
        resetMode: "barcode-only"
      });
    } catch (error) {
      const normalizedError = normalizeLookupError(error);
      setProdutoValidationState("invalid");
      ...
      showValidationPopup({
        ...
        detail: `Barra ${value} | Código interno ${codigoInternoAtual}`,
        detailsList: [
          `Código interno informado: ${codigoInternoAtual}`,
          `Barra lida: ${value}`
        ],
  ...
  }, [codigoInternoAtual, currentCd, focusProduto, isOnline, preferOfflineMode, queueValidationAudit, showValidationPopup, triggerScanErrorAlert]);
  ...
  const commitScannerInput = useCallback(async (target: ScannerInputTarget, rawValue: string) => {
    const normalized = normalizeBarcodeInput(rawValue);
    ...
    if (target === "codigoInterno") {
      ...
      setCodigoInternoInput(normalized);
      await commitCodigoInternoInput(normalized);
      return;
    }
  ...
  const onScanInputChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (activeInputTarget === "produto") {
      ...
      setCodigoInternoInput(nextValue);
      setCodigoInternoValidationState("idle");
    }
  ...
  useEffect(() => {
    ...
    const loadPreferences = async () => {
      const prefs = await getValidarEtiquetaPulmaoPreferences(profile.user_id);
      if (!mounted) return;
  ...
  useEffect(() => {
    ...
    void saveValidarEtiquetaPulmaoPreferences(profile.user_id, {
  ...
  useEffect(() => {
    return () => {
      ...
      for (const target of ["produto", "codigoInterno"] as const) {
        ...
        if (nativeBarcodeDetectorCtor && typeof navigator.mediaDevices?.getUserMedia === "function") {
          try {
            const detector = new nativeBarcodeDetectorCtor({
              formats: scannerTarget === "codigoInterno"
                ? ["qr_code", "code_128", "code_39", "ean_13", "ean_8", "upc_a", "upc_e", "itf", "codabar"]
                : ["code_128", "code_39", "ean_13", "ean_8", "upc_a", "upc_e", "itf", "codabar"]
            });
        ...
        const controls = await reader.decodeFromConstraints(
          ...
          (scanResult, error) => {
            ...
            if (scanResult) {
              ...
              if (scannerTarget !== "codigoInterno" && /QR_CODE/i.test(formatName)) return;
    ...
    if (next) {
      if (!isOnline) {
        if (hasLocalBase) {
          ...
          setErrorMessage("Sem base local. Conecte-se para sincronizar db_barras.");
        }
        return;
      }
  ...
  return (
    <>
      ...
      <section className="modules-shell validar-end-shell">
            ...
            <label>
              ...
              <div className="input-icon-wrap with-action">
                <span className={activeIconClassName} aria-hidden="true">
                  {activeInputTarget === "produto" ? barcodeIcon() : qrIcon()}
                </span>
                <input
                  ref={produtoRef}
                  type="text"
                  ...
                  onFocus={() => {
                    ...
                    if (activeInputTarget === "produto") {
                      ...
                      enableCodigoInternoSoftKeyboard();
                    }
                  }}
                  onPointerDown={() => {
                    unlockAudioContextFromGesture();
                ...
                <button
                  ...
                  title={activeInputTarget === "produto" ? "Ler código de barras pela câmera" : "Ler QR Code pela câmera"}
                  aria-label={activeInputTarget === "produto" ? "Ler código de barras pela câmera" : "Ler QR Code pela câmera"}
        ...
        {codigoInternoAtual != null ? (
          <article className="validar-end-product-card">
            <h3>Validação em andamento</h3>
            <p><strong>Código interno informado:</strong> {codigoInternoAtual}</p>
            <p><strong>Status:</strong> {currentProduct ? "Código de barras localizado" : "Aguardando leitura do código de barras"}</p>
            {currentProduct ? (
              <>
                <p><strong>Descrição:</strong> {currentProduct.descricao || "-"}</p>
                <p><strong>CODDV encontrado:</strong> {currentProduct.coddv}</p>
                ...
                <div className={`validar-end-popup validar-end-popup-${validationPopup.tone}`}>
                  ...
                  {validationPopup.tone === "error" ? (
                    <div className="validar-end-popup-sep-wrap">
                      ...
                      {validationPopup.detailsList.length > 0 ? (
                        <ul>
                          {validationPopup.detailsList.map((item) => <li key={item}>{item}</li>)}
                        </ul>
                      ) : (
                ...
                <div className="scanner-dialog surface-enter" onClick={(event) => event.stopPropagation()}>
                  <div className="scanner-head">
                    <h3 id="validar-end-scanner-title">
                      {scannerTarget === "produto" ? "Scanner do código de barras" : "Scanner do código interno"}
                    </h3>
                  ...
                  <p className="scanner-hint">
                    {scannerTarget === "produto"
                      ...
                      : "Aponte para o QR Code do código interno."}
                  </p>
