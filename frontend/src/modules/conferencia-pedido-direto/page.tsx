...
export default function ConferenciaPedidoDiretoPage({ isOnline, profile }: ConferenciaPedidoDiretoPageProps) {
  ...
  const [pendingSyncRows, setPendingSyncRows] = useState<PedidoDiretoLocalVolume[]>([]);
  ...
  const [reportCount, setReportCount] = useState<PedidoDiretoReportCount | null>(null);
  ...
  const buildReadOnlyVolumeSignature = useCallback((volume: PedidoDiretoLocalVolume) => JSON.stringify({
    ...
    key: volume.local_key ?? volume.remote_conf_id ?? volume.id_vol
  }), []);
  ...
  const dismissReadOnlyVolume = useCallback((volume: PedidoDiretoLocalVolume) => {
    if (typeof window === "undefined") return;
  ...
  const isDismissedReadOnlyVolume = useCallback((volume: PedidoDiretoLocalVolume) => {
  ...
  const runReportSearch = useCallback(async () => {
    ...
    try {
      const count = await countPedidoDiretoReportRows(filters);
  ...
  const runReportExport = useCallback(async () => {
    ...
    try {
      ...
      const itemRows: PedidoDiretoReportRow[] = [];
      let offset = 0;

      while (offset < expectedItems) {
        const batch = await fetchPedidoDiretoReportRows(filters, offset, REPORT_PAGE_SIZE);
      ...
      const summarySheet = XLSX.utils.aoa_to_sheet([
        ["Relatório de Conferência de Pedido Direto"],
      ...
      XLSX.writeFile(
        workbook,
        `relatorio-conferencia-pedido-direto-${filters.dtIni}-${filters.dtFim}-cd${String(filters.cd).padStart(2, "0")}.xlsx`,
        { compression: true }
      );
        ...
        {showReportPanel && canSeeReportTools ? (
          <section className="termo-report-panel">
            <div className="termo-report-head">
              <h3>Relatório de Conferência de Pedido Direto</h3>
