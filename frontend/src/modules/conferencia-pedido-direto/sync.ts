...
function mapManifestMeta(raw: Record<string, unknown>): PedidoDiretoManifestMeta {
  return {
    ...
    volumes_count: parseInteger(raw.volumes_count ?? raw.etiquetas_count),
...
function mapManifestItem(raw: Record<string, unknown>): PedidoDiretoManifestItemRow {
  return {
...
function mapManifestBarras(raw: Record<string, unknown>): PedidoDiretoManifestBarrasRow {
  return {
...
function mapVolume(raw: Record<string, unknown>): PedidoDiretoVolumeRow {
...
function mapItem(raw: Record<string, unknown>): PedidoDiretoItemRow {
...
function mapReportCount(raw: Record<string, unknown>): PedidoDiretoReportCount {
  return {
...
function mapReportRow(raw: Record<string, unknown>): PedidoDiretoReportRow {
  const statusRaw = String(raw.status ?? "em_conferencia");
  const status = statusRaw === "finalizado_ok" || statusRaw === "finalizado_falta"
    ? statusRaw
    : "em_conferencia";
...
export async function fetchManifestMeta(cd: number): Promise<PedidoDiretoManifestMeta> {
  if (!supabase) throw new Error("Supabase não inicializado.");
  const { data, error } = await supabase.rpc("rpc_conf_pedido_direto_manifest_meta", {
    p_cd: cd
  });
  if (error) throw new Error(toErrorMessage(error));
  const first = Array.isArray(data) ? (data[0] as Record<string, unknown> | undefined) : undefined;
  if (!first) throw new Error("Manifesto do pedido direto não encontrado.");
...
export async function fetchManifestItemsPage(
  ...
  if (!supabase) throw new Error("Supabase não inicializado.");
  const { data, error } = await supabase.rpc("rpc_conf_pedido_direto_manifest_items_page", {
    p_cd: cd,
  ...
  return data
    ...
    .filter((row) => row.id_vol && row.coddv > 0);
}
...
): Promise<PedidoDiretoManifestBarrasRow[]> {
  if (!supabase) throw new Error("Supabase não inicializado.");
  const { data, error } = await supabase.rpc("rpc_conf_pedido_direto_manifest_barras_page", {
    p_cd: cd,
    p_offset: Math.max(offset, 0),
    p_limit: Math.max(1, limit)
  });
...
export async function fetchVolumeItems(confId: string): Promise<PedidoDiretoItemRow[]> {
  ...
  const fallbackNeeded = /rpc_conf_pedido_direto_get_items_v2/i.test(toErrorMessage(v2Error));
  if (!fallbackNeeded) {
    throw new Error(toErrorMessage(v2Error));
  }

  const { data, error } = await supabase.rpc("rpc_conf_pedido_direto_get_items", {
    p_conf_id: confId
  });
  if (error) throw new Error(toErrorMessage(error));
  if (!Array.isArray(data)) return [];
...
export async function scanBarcode(confId: string, barras: string, qtd: number): Promise<PedidoDiretoItemRow> {
  if (!supabase) throw new Error("Supabase não inicializado.");
  const { data, error } = await supabase.rpc("rpc_conf_pedido_direto_scan_barcode", {
    p_conf_id: confId,
...
export async function setItemQtd(confId: string, coddv: number, qtdConferida: number): Promise<PedidoDiretoItemRow> {
  if (!supabase) throw new Error("Supabase não inicializado.");
  const { data, error } = await supabase.rpc("rpc_conf_pedido_direto_set_item_qtd", {
    p_conf_id: confId,
    p_coddv: coddv,
...
export async function resetItem(confId: string, coddv: number): Promise<PedidoDiretoItemRow> {
  if (!supabase) throw new Error("Supabase não inicializado.");
  const { data, error } = await supabase.rpc("rpc_conf_pedido_direto_reset_item", {
    p_conf_id: confId,
    p_coddv: coddv
  });
  if (error) throw new Error(toErrorMessage(error));
...
export async function syncSnapshot(
  ...
  const { error } = await supabase.rpc("rpc_conf_pedido_direto_sync_snapshot", {
    p_conf_id: confId,
    p_items: payload
  });
  if (error) throw new Error(toErrorMessage(error));
...
export async function finalizeVolume(
  ...
  if (!supabase) throw new Error("Supabase não inicializado.");
  const { data, error } = await supabase.rpc("rpc_conf_pedido_direto_finalize", {
    p_conf_id: confId,
...
export async function cancelVolume(confId: string): Promise<boolean> {
  if (!supabase) throw new Error("Supabase não inicializado.");
  const { data, error } = await supabase.rpc("rpc_conf_pedido_direto_cancel", {
    p_conf_id: confId
  });
  if (error) throw new Error(toErrorMessage(error));
  const first = Array.isArray(data) ? (data[0] as Record<string, unknown> | undefined) : undefined;
...
export async function countPedidoDiretoReportRows(
  filters: PedidoDiretoReportFilters
): Promise<PedidoDiretoReportCount> {
  if (!supabase) throw new Error("Supabase não inicializado.");
...
export async function fetchPedidoDiretoReportRows(
  filters: PedidoDiretoReportFilters,
  offset = 0,
  limit = 1000
): Promise<PedidoDiretoReportRow[]> {
  if (!supabase) throw new Error("Supabase não inicializado.");

  const { data, error } = await supabase.rpc("rpc_conf_pedido_direto_report_rows", {
    p_dt_ini: filters.dtIni,
    p_dt_fim: filters.dtFim,
    p_cd: filters.cd,
    p_offset: Math.max(offset, 0),
