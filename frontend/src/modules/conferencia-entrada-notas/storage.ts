import type {
  ...
  EntradaNotasPendingSummary,
  EntradaNotasPreferences,
  EntradaNotasRouteOverviewRow
} from "./types";

const DB_NAME = "auditoria-entrada-notas-v1";
const DB_VERSION = 1;
...
interface ManifestItemStoreRow extends EntradaNotasManifestItemRow {
  key: string;
  user_id: string;
  cd: number;
}

interface ManifestBarrasStoreRow extends EntradaNotasManifestBarrasRow {
  key: string;
  user_id: string;
  cd: number;
}

interface ManifestMetaStoreRow extends EntradaNotasManifestMeta {
  key: string;
  user_id: string;
  cached_at: string;
}
...
interface RouteOverviewStoreRow {
  ...
  user_id: string;
  cd: number;
  rows: EntradaNotasRouteOverviewRow[];
  updated_at: string;
}

interface PrefStoreRow {
  key: string;
  value: EntradaNotasPreferences;
}
...
function normalizePrefs(value: Partial<EntradaNotasPreferences> | null | undefined): EntradaNotasPreferences {
  return {
...
export function buildEntradaNotasConferenceKey(userId: string, cd: number, confDate: string, idEtiqueta: string): string {
  return `volume:${userId}:${cd}:${confDate}:${idEtiqueta}`;
}

export function buildEntradaNotasVolumeKey(userId: string, cd: number, confDate: string, idEtiqueta: string): string {
  return buildEntradaNotasConferenceKey(userId, cd, confDate, idEtiqueta);
}
  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(STORE_MANIFEST_ITEMS)) {
        ...
        store.createIndex(INDEX_ITEMS_BY_USER_CD_ETIQUETA, ["user_id", "cd", "nr_volume"], { unique: false });
      }
      ...
      if (!db.objectStoreNames.contains(STORE_VOLUMES)) {
        ...
        store.createIndex(INDEX_VOLUMES_BY_USER_CD_ETIQUETA, ["user_id", "cd", "nr_volume"], { unique: false });
...
function sortVolumes(rows: EntradaNotasLocalVolume[]): EntradaNotasLocalVolume[] {
...
export async function getEntradaNotasPreferences(userId: string): Promise<EntradaNotasPreferences> {
  const db = await getDb();
...
export async function saveEntradaNotasPreferences(userId: string, value: EntradaNotasPreferences): Promise<void> {
  const db = await getDb();
...
export async function saveManifestSnapshot(params: {
...

  {
    ...
    for (const row of params.items) {
      const idEtiqueta = String(row.nr_volume ?? "").trim();
...
export async function findManifestBarras(
  ...
  cd: number,
  barras: string
): Promise<EntradaNotasManifestBarrasRow | null> {
...
export async function getRouteOverviewLocal(userId: string, cd: number): Promise<EntradaNotasRouteOverviewRow[]> {
  const db = await getDb();
...
export async function saveLocalVolume(volume: EntradaNotasLocalVolume): Promise<void> {
  const db = await getDb();
...
export async function getLocalVolume(
  ...
  const store = transaction.objectStore(STORE_VOLUMES);
  const key = buildEntradaNotasConferenceKey(userId, cd, confDate, idEtiqueta.trim());
  const raw = await requestToPromise(store.get(key));
  await transactionDone(transaction);
  return (raw as EntradaNotasLocalVolume | undefined) ?? null;
}
...
export async function getLatestLocalVolumeByEtiqueta(
  ...
  const rows = (await requestToPromise(
    index.getAll(IDBKeyRange.only([userId, cd, etiqueta]))
  )) as EntradaNotasLocalVolume[];
  await transactionDone(transaction);
  if (!rows.length) return null;
  return sortVolumes(rows)[0];
}

export async function listUserLocalVolumes(userId: string): Promise<EntradaNotasLocalVolume[]> {
  const db = await getDb();
  const transaction = db.transaction(STORE_VOLUMES, "readonly");
  const index = transaction.objectStore(STORE_VOLUMES).index(INDEX_VOLUMES_BY_USER);
  const rows = (await requestToPromise(index.getAll(IDBKeyRange.only(userId)))) as EntradaNotasLocalVolume[];
  await transactionDone(transaction);
  return sortVolumes(rows);
}

export async function listPendingLocalVolumes(userId: string): Promise<EntradaNotasLocalVolume[]> {
...
export async function getPendingSummary(userId: string): Promise<EntradaNotasPendingSummary> {
...
export async function cleanupExpiredEntradaNotasVolumes(userId: string): Promise<number> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      const row = cursor.value as EntradaNotasLocalVolume;
...
export async function cleanupExpiredEntradaNotasConferences(userId: string): Promise<number> {
  return cleanupExpiredEntradaNotasVolumes(userId);
}

export async function clearUserEntradaNotasSessionCache(userId: string): Promise<void> {
