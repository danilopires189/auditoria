import type {
  ...
  DevolucaoMercadoriaPendingSummary,
  DevolucaoMercadoriaPreferences,
  DevolucaoMercadoriaRouteOverviewRow
} from "./types";

const DB_NAME = "auditoria-devolucao-v1";
const DB_VERSION = 1;
...
interface ManifestItemStoreRow extends DevolucaoMercadoriaManifestItemRow {
  key: string;
  user_id: string;
  cd: number;
}

interface ManifestBarrasStoreRow extends DevolucaoMercadoriaManifestBarrasRow {
  key: string;
  user_id: string;
  cd: number;
}

interface ManifestMetaStoreRow extends DevolucaoMercadoriaManifestMeta {
  key: string;
  user_id: string;
  cached_at: string;
}
...
interface RouteOverviewStoreRow {
  ...
  user_id: string;
  cd: number;
  rows: DevolucaoMercadoriaRouteOverviewRow[];
  updated_at: string;
}

interface PrefStoreRow {
  key: string;
  value: DevolucaoMercadoriaPreferences;
}
...
function normalizePrefs(value: Partial<DevolucaoMercadoriaPreferences> | null | undefined): DevolucaoMercadoriaPreferences {
  return {
...
export function buildDevolucaoMercadoriaVolumeKey(userId: string, cd: number, confDate: string, idEtiqueta: string): string {
  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(STORE_MANIFEST_ITEMS)) {
        ...
        store.createIndex(INDEX_ITEMS_BY_USER_CD_ETIQUETA, ["user_id", "cd", "ref"], { unique: false });
      }
      ...
      if (!db.objectStoreNames.contains(STORE_VOLUMES)) {
        ...
        store.createIndex(INDEX_VOLUMES_BY_USER_CD_ETIQUETA, ["user_id", "cd", "ref"], { unique: false });
...
function sortVolumes(rows: DevolucaoMercadoriaLocalVolume[]): DevolucaoMercadoriaLocalVolume[] {
...
export async function getDevolucaoMercadoriaPreferences(userId: string): Promise<DevolucaoMercadoriaPreferences> {
  const db = await getDb();
...
export async function saveDevolucaoMercadoriaPreferences(userId: string, value: DevolucaoMercadoriaPreferences): Promise<void> {
  const db = await getDb();
...
export async function saveManifestSnapshot(params: {
...

  {
    ...
    for (const row of params.items) {
      const idEtiqueta = String(row.ref ?? "").trim();
...

  {
    ...
    const metaPayload: ManifestMetaStoreRow = {
      ...
      etiquetas_count: parsePositiveInt(params.meta.etiquetas_count, 0),
...
export async function listManifestItemsByCd(
  ...
  return rows
    ...
    .sort((a, b) => {
      const byVolume = a.ref.localeCompare(b.ref, "pt-BR", { numeric: true, sensitivity: "base" });
...
export async function listManifestVolumes(
  userId: string,
  cd: number
): Promise<DevolucaoMercadoriaManifestVolumeRow[]> {
  const db = await getDb();
  const transaction = db.transaction(STORE_MANIFEST_ITEMS, "readonly");
  const store = transaction.objectStore(STORE_MANIFEST_ITEMS);
  const index = store.index(INDEX_ITEMS_BY_USER_CD);
  ...
  const grouped = new Map<string, DevolucaoMercadoriaManifestVolumeRow>();
  for (const row of rows) {
    const nrVolume = String(row.ref ?? "").trim();
...
export async function findManifestBarras(
  ...
  cd: number,
  barras: string
): Promise<DevolucaoMercadoriaManifestBarrasRow | null> {
...
export async function getRouteOverviewLocal(userId: string, cd: number): Promise<DevolucaoMercadoriaRouteOverviewRow[]> {
  const db = await getDb();
...
export async function saveLocalVolume(volume: DevolucaoMercadoriaLocalVolume): Promise<void> {
  const db = await getDb();
...
export async function getLocalVolume(
  ...
  confDate: string,
  idEtiqueta: string
): Promise<DevolucaoMercadoriaLocalVolume | null> {
  const db = await getDb();
  const transaction = db.transaction(STORE_VOLUMES, "readonly");
  const store = transaction.objectStore(STORE_VOLUMES);
  const key = buildDevolucaoMercadoriaVolumeKey(userId, cd, confDate, idEtiqueta.trim());
  const raw = await requestToPromise(store.get(key));
  await transactionDone(transaction);
  return (raw as DevolucaoMercadoriaLocalVolume | undefined) ?? null;
}
...
export async function getLatestLocalVolumeByEtiqueta(
  ...
  const rows = (await requestToPromise(
    index.getAll(IDBKeyRange.only([userId, cd, etiqueta]))
  )) as DevolucaoMercadoriaLocalVolume[];
  await transactionDone(transaction);
  if (!rows.length) return null;
  return sortVolumes(rows)[0];
}

export async function listUserLocalVolumes(userId: string): Promise<DevolucaoMercadoriaLocalVolume[]> {
  const db = await getDb();
  const transaction = db.transaction(STORE_VOLUMES, "readonly");
  const index = transaction.objectStore(STORE_VOLUMES).index(INDEX_VOLUMES_BY_USER);
  const rows = (await requestToPromise(index.getAll(IDBKeyRange.only(userId)))) as DevolucaoMercadoriaLocalVolume[];
  await transactionDone(transaction);
  return sortVolumes(rows);
}

export async function listPendingLocalVolumes(userId: string): Promise<DevolucaoMercadoriaLocalVolume[]> {
...
export async function getPendingSummary(userId: string): Promise<DevolucaoMercadoriaPendingSummary> {
...
export async function cleanupExpiredDevolucaoMercadoriaVolumes(userId: string): Promise<number> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      const row = cursor.value as DevolucaoMercadoriaLocalVolume;
...
export async function clearUserDevolucaoMercadoriaSessionCache(userId: string): Promise<void> {
