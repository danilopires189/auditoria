...
create index if not exists idx_aud_ronda_quality_sessions_month_cd_type_zona_coluna
    on app.aud_ronda_quality_sessions(month_ref, cd, zone_type, zona, coluna, created_at desc);

update app.aud_ronda_quality_sessions s
set coluna = x.coluna
from (
    select
        o.audit_id,
        min(o.coluna) as coluna
    from app.aud_ronda_quality_occurrences o
    where o.zone_type = 'PUL'
      and o.coluna is not null
    group by o.audit_id
    having count(distinct o.coluna) = 1
) x
where s.audit_id = x.audit_id
  and s.zone_type = 'PUL'
...
drop function if exists public.rpc_ronda_quality_submit_audit(integer, text, text, text, jsonb);
