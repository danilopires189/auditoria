-- V441: Módulo Controle de Caixa Térmica
-- Tabelas, RPCs de CRUD e integração com produtividade

-- ============================================================
-- TABELAS
-- ============================================================

create table if not exists app.controle_caixa_termica (
    ...
    status       text not null default 'disponivel'
                 check (status in ('disponivel', 'em_transito')),
    created_at   timestamptz not null default now(),
    created_by   uuid not null references auth.users(id),
    updated_at   timestamptz not null default now(),
    constraint uq_caixa_termica_cd_codigo unique (cd, codigo)
);

create table if not exists app.controle_caixa_termica_movs (
    id               uuid primary key default gen_random_uuid(),
    caixa_id         uuid not null references app.controle_caixa_termica(id),
    tipo             text not null check (tipo in ('expedicao', 'recebimento')),
    cd               integer not null,
    ...
    user_id_resp     uuid not null references auth.users(id),
    mat_resp         text not null,
    nome_resp        text not null,
    data_hr          timestamptz not null default now(),
    created_at       timestamptz not null default now()
);

create index if not exists idx_caixa_termica_movs_caixa_data
    on app.controle_caixa_termica_movs (caixa_id, data_hr desc);

create index if not exists idx_caixa_termica_movs_cd_data
    on app.controle_caixa_termica_movs (cd, data_hr desc);

create index if not exists idx_caixa_termica_cd_status
    on app.controle_caixa_termica (cd, status);

-- ============================================================
-- RPC: listar caixas do CD com última movimentação
-- ============================================================
...
returns table (
    ...
    last_mov_tipo       text,
    last_mov_data_hr    timestamptz,
    last_mov_placa      text,
    last_mov_rota       text,
    last_mov_filial     integer,
    last_mov_filial_nome text
)
language plpgsql
stable
security definer
...
begin
    ...
    select
        ...
        c.created_by,
        c.updated_at,
        m.tipo            as last_mov_tipo,
        m.data_hr         as last_mov_data_hr,
        m.placa           as last_mov_placa,
        m.rota            as last_mov_rota,
        m.filial          as last_mov_filial,
        m.filial_nome     as last_mov_filial_nome
...
begin
    ...
    return query
    select
        c.id, c.cd, c.codigo, c.descricao, c.observacoes, c.status,
        c.created_at, c.created_by, c.updated_at,
        m.tipo, m.data_hr, m.placa, m.rota, m.filial, m.filial_nome
    from app.controle_caixa_termica c
    left join lateral (
        select mv.tipo, mv.data_hr, mv.placa, mv.rota, mv.filial, mv.filial_nome
        from app.controle_caixa_termica_movs mv
    ...
    where c.cd = v_cd
      and upper(trim(c.codigo)) = upper(trim(p_codigo))
    limit 1;
end;
$$;
...
returns table (
    ...
    mov_id          uuid,
    mov_tipo        text,
    mov_data_hr     timestamptz,
    mov_placa       text,
    mov_rota        text,
    mov_filial      integer,
    mov_filial_nome text,
    mov_etiqueta_volume text
)
language plpgsql
security definer
set search_path = app, authz, public
...
begin
    ...
    set status = 'em_transito', updated_at = v_now
    where id = p_caixa_id;
    ...
    insert into app.controle_caixa_termica_movs (
        ...
        p_caixa_id, 'expedicao', v_cd,
        nullif(trim(coalesce(p_etiqueta_volume, '')), ''),
        p_filial,
        nullif(trim(coalesce(p_filial_nome, '')), ''),
        nullif(trim(coalesce(p_rota, '')), ''),
...
begin
    ...
    update app.controle_caixa_termica
    set status = 'disponivel', updated_at = v_now
    where id = p_caixa_id;
...
-- ============================================================
-- PRODUTIVIDADE: incluir caixa_termica_mov em events_base
-- ============================================================
...
as $function$
    ...
    -- Pontuação por posição diária do usuário no módulo caixa_termica
    caixa_termica_ranked as (
        select
            m.id,
            m.caixa_id,
            m.tipo,
            ...
            greatest(0.5, 3.5 - (
                (row_number() over (
                    partition by m.cd, m.user_id_resp,
                                 timezone('America/Sao_Paulo', m.data_hr)::date
                    order by m.data_hr
                ) - 1) * 0.5
            ))::numeric(18,3) as pontos
        from app.controle_caixa_termica_movs m
        join app.controle_caixa_termica c on c.id = m.caixa_id
        where m.cd = p_cd
          and (p_dt_ini is null or timezone('America/Sao_Paulo', m.data_hr)::date >= p_dt_ini)
          and (p_dt_fim is null or timezone('America/Sao_Paulo', m.data_hr)::date <= p_dt_fim)
    )
    select
        e.activity_key,
        e.activity_label,
    ...
    from (
        ...
        -- Caixa Térmica: pontos por posição diária do usuário
        select
            'caixa_termica_mov'::text as activity_key,
            'Caixa Térmica'::text as activity_label,
            'pontos'::text as unit_label,
            ct.user_id_resp as user_id,
            ct.mat_resp as mat,
            ct.nome_resp as nome,
            timezone('America/Sao_Paulo', ct.data_hr)::date as event_date,
            ct.pontos as metric_value,
            concat_ws(
                ' | ',
                case ct.tipo when 'expedicao' then 'Expedição' else 'Recebimento' end,
                ct.codigo,
                case
                    when nullif(trim(coalesce(ct.rota, '')), '') is not null
                        then format('Rota %s', ct.rota)
                    else null
                end
            ) as detail,
            ct.id::text as source_ref,
            ct.data_hr as event_at
        from caixa_termica_ranked ct
    ) e;
$function$;


-- ============================================================
-- Atualizar RPCs de produtividade para incluir caixa_termica
-- ============================================================

-- Atualizar rpc_produtividade_activity_totals: adicionar caixa_termica ao catálogo
    ...
    with catalog as (
        select * from (
            values
                ...
                (16, 'caixa_termica_mov', 'Caixa Térmica', 'pontos')
...
-- Atualizar rpc_produtividade_entries: adicionar caixa_termica_mov como activity_key válida
...
-- ============================================================
-- Atualizar rpc_produtividade_ranking: adicionar caixa_termica
-- ============================================================
...
begin
    ...
    componentes as (
        select
            ...
            -- caixa_termica: pontos são a soma direta dos metric_values (já computados com posição diária)
