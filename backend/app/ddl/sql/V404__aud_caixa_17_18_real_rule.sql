...
set
    pedido = left(etiqueta, 7)::bigint,
    data_pedido = make_date(left(etiqueta, 4)::integer, 1, 1)
        + (substring(etiqueta from 5 for 3)::integer - 1),
    dv = app.aud_caixa_strip_leading_zeros(substring(etiqueta from 8 for 1)),
    filial = app.aud_caixa_strip_leading_zeros(
        case
            when char_length(etiqueta) = 17 then right(etiqueta, 3)
            else right(etiqueta, 4)
        end
    )::bigint,
    volume = case
