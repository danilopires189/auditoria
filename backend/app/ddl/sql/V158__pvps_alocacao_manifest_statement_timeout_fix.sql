create index if not exists idx_app_db_end_alocacao_lookup_expr
    on app.db_end (
        cd,
        coddv,
    ...
    where nullif(trim(coalesce(validade, '')), '') is not null;
...
begin
    ...
    -- Avoid expensive full reseed on every page request.
