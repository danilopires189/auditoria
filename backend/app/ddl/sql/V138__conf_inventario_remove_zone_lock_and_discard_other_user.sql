...
begin
    ...
    if v_event_type = 'count_upsert' then
        ...
        if v_etapa = 1 and v_c2.count_id is not null then
            if v_c2.counted_by <> v_uid then
                if p_client_event_id is not null then
                    insert into app.conf_inventario_event_log (client_event_id, user_id, event_type, payload, status, info)
                    values (p_client_event_id, v_uid, v_event_type, v_payload, 'accepted', 'COUNT_DISCARDED_OUTRO_USUARIO');
                end if;
                return query select true, 'COUNT_DISCARDED_OUTRO_USUARIO', now();
                return;
            end if;
        ...
        if v_etapa = 2 then
            ...
            if v_c2.count_id is not null and v_c2.counted_by <> v_uid then
                if p_client_event_id is not null then
                    insert into app.conf_inventario_event_log (client_event_id, user_id, event_type, payload, status, info)
                    values (p_client_event_id, v_uid, v_event_type, v_payload, 'accepted', 'COUNT_DISCARDED_OUTRO_USUARIO');
                end if;
