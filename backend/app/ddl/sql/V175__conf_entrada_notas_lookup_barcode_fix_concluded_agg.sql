...
begin
    ...
    into v_has_pending_base;

    with concluded as (
        select
            c.seq_entrada,
            c.nf,
            coalesce(nullif(trim(c.started_nome), ''), nullif(trim(c.started_mat), ''), 'usuario nao identificado') as actor,
            coalesce(c.finalized_at, c.updated_at, c.started_at) as status_at,
            row_number() over (
                order by coalesce(c.finalized_at, c.updated_at, c.started_at) desc nulls last, c.seq_entrada, c.nf
            ) as rn
        from app.conf_entrada_notas c
        join app.conf_entrada_notas_itens_conferidos i
        ...
        format(
            ...
            string_agg(
                format(
                    ...
                    c.nf,
                    c.actor,
                    to_char(coalesce(c.status_at, now()) at time zone 'America/Sao_Paulo', 'DD/MM/YYYY HH24:MI:SS')
                ),
                '. '
                order by c.seq_entrada, c.nf
            ),
            case
                when max(t.total_rows) > 3 then format('. mais %s Seq/NF concluidas', max(t.total_rows) - 3)
                else ''
            end
        )
    ...
    if not v_has_pending_base then
