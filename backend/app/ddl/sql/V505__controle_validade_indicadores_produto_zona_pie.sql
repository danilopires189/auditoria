...
begin
    ...
    base_atual as (
        ...
        group by de.cd, de.coddv, app.pvps_alocacao_normalize_zone(de.endereco)
    ),
    coletados_mes as (
        select distinct
            col.cd,
    ...
    select
        status.zona_key,
        count(*) filter (where status.coletado)::integer as coletado_total,
        count(*) filter (where not status.coletado)::integer as pendente_total,
