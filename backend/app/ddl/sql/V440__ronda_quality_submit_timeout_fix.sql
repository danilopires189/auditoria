create index if not exists idx_app_db_end_ronda_zone_lookup
    on app.db_end (
        cd,
        upper(trim(coalesce(tipo, ''))),
        app.ronda_quality_normalize_zone(endereco, tipo)
    )
    where coddv is not null
      and coddv > 0
      and nullif(trim(coalesce(endereco, '')), '') is not null;

create index if not exists idx_app_db_end_ronda_address_lookup
    on app.db_end (
        cd,
        upper(trim(coalesce(tipo, ''))),
        upper(trim(endereco)),
        app.ronda_quality_normalize_zone(endereco, tipo),
        app.ronda_quality_normalize_column(endereco)
    )
    where coddv is not null
      and coddv > 0
      and nullif(trim(coalesce(endereco, '')), '') is not null;

create index if not exists idx_app_db_estq_entr_ronda_available
    on app.db_estq_entr (cd, coddv)
    where coalesce(qtd_est_disp, 0) > 0;
...
begin
    ...
    if not exists (
        ...
        where d.cd = v_cd
          ...
          and app.ronda_quality_normalize_zone(d.endereco, d.tipo) = v_zona
          and d.coddv is not null
          and d.coddv > 0
          and nullif(trim(coalesce(d.endereco, '')), '') is not null
          and exists (
              select 1
              from app.db_estq_entr st
              where st.cd = d.cd
                and st.coddv = d.coddv
                and coalesce(st.qtd_est_disp, 0) > 0
          )
        limit 1
    ) then
        ...
        loop
            ...
            if v_endereco_manual then
                ...
                if not exists (
                    ...
                    where d.cd = v_cd
                      and upper(trim(coalesce(d.tipo, ''))) = v_zone_type
                      and upper(trim(d.endereco)) = v_endereco
                      and app.ronda_quality_normalize_zone(d.endereco, d.tipo) = v_zona
                      and (
                          v_zone_type = 'SEP'
                          or app.ronda_quality_normalize_column(d.endereco) = v_session_coluna
                      )
                      and d.coddv is not null
                      and d.coddv > 0
                      and exists (
