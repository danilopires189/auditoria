alter table staging.db_prod_vol
    add column if not exists seq_ped text,
    add column if not exists filial bigint,
    add column if not exists fili_nm_fantasia text,
    add column if not exists vol_exp integer,
    add column if not exists usuario text,
    add column if not exists dt_ped timestamptz,
    add column if not exists dt_lib timestamptz,
    add column if not exists placa text,
    add column if not exists rota integer,
    add column if not exists descricao text,
    add column if not exists encerramento timestamptz;
...
begin
    if exists (
        select 1
        from pg_constraint
        where conname = 'uq_app_db_prod_vol'
    ) then
        alter table app.db_prod_vol
            drop constraint uq_app_db_prod_vol;
    end if;
end $$;

delete from app.db_prod_vol
where seq_ped is null
  and filial is null
  and usuario is null
  and encerramento is null;

delete from staging.db_prod_vol;

create index if not exists idx_app_db_prod_vol_cd_aud
    on app.db_prod_vol(cd, aud);

create index if not exists idx_app_db_prod_vol_cd_event_at
    on app.db_prod_vol(cd, encerramento, dt_lib, dt_ped);
