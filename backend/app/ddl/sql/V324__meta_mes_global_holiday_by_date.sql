...
select
    ...
    src.updated_at
from (
    select distinct on (t.cd, t.date_ref)
        t.cd,
        t.date_ref,
        t.updated_by,
        t.updated_mat,
    ...
    where coalesce(t.is_holiday, false)
    order by t.cd, t.date_ref, t.updated_at desc
) src
cross join app.meta_mes_activity_catalog() catalog
