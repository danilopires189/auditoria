...
begin
    ...
    new.data_pedido := make_date(left(v_pedido_text, 4)::integer, 1, 1) + (v_day_num - 1);
...
set data_pedido = case
    ...
    else make_date(left(lpad(pedido::text, 7, '0'), 4)::integer, 1, 1)
        + (substring(lpad(pedido::text, 7, '0') from 5 for 3)::integer - 1)
end
