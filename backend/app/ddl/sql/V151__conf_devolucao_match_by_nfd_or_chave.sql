-- Devolucao: status e retomada devem casar por NFD ou Chave (priorizando chave).
-- Garante verificacao historica em toda db_devolucao, sem depender do dia atual.
...
begin
    ...
    conf as (
        select
            c.*,
            nullif(trim(coalesce(c.chave, '')), '') as chave_norm
    ...
    left join lateral (
        ...
        where (
                n.chave is not null
            and c_match.chave_norm = n.chave
        )
           or (
                n.nfd is not null
            and c_match.nfd = n.nfd
        )
        order by
            case
                when n.chave is not null and c_match.chave_norm = n.chave then 0
                when n.nfd is not null and c_match.nfd = n.nfd then 1
                else 2
            end,
            case when c_match.status = 'em_conferencia' then 0 else 1 end,
            coalesce(c_match.finalized_at, c_match.updated_at, c_match.started_at) desc nulls last,
            c_match.updated_at desc nulls last,
            c_match.started_at desc nulls last,
            c_match.conf_date desc
        limit 1
    ) c on true
    order by n.ref;
end;
...
begin
    ...
    where c.cd = v_cd
      ...
      and (
            (
                v_match_chave is not null
            and nullif(trim(coalesce(c.chave, '')), '') = v_match_chave
            )
         or (
                v_match_nfd is not null
            and c.nfd = v_match_nfd
            )
      )
    order by
        case
            when v_match_chave is not null and nullif(trim(coalesce(c.chave, '')), '') = v_match_chave then 0
            when v_match_nfd is not null and c.nfd = v_match_nfd then 1
            else 2
        end,
