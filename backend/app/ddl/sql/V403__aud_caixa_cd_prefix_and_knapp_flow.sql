...
declare
    ...
    v_route_nome text;
    v_route_uf text;
    v_cd_prefix_text text;
    v_cd_prefix_len integer;
    v_filial_len integer;
begin
    v_uid := auth.uid();
    ...
    if v_len in (17, 18) then
        v_cd_prefix_text := trim(new.cd::text);
        v_cd_prefix_len := char_length(v_cd_prefix_text);
        if left(new.etiqueta, v_cd_prefix_len) <> v_cd_prefix_text then
            raise exception 'ETIQUETA_CD_PREFIXO_INVALIDO';
        end if;

        v_filial_len := v_len - v_cd_prefix_len - 13;
        if v_filial_len not in (3, 4) then
            raise exception 'ETIQUETA_17_18_ESTRUTURA_INVALIDA';
        end if;

        v_pedido_text := substring(new.etiqueta from v_cd_prefix_len + 1 for 7);
        v_dv_text := substring(new.etiqueta from v_cd_prefix_len + 8 for 6);
        v_filial_text := right(new.etiqueta, v_filial_len);
...
set
    pedido = substring(etiqueta from char_length(cd::text) + 1 for 7)::bigint,
    data_pedido = make_date(substring(etiqueta from char_length(cd::text) + 1 for 4)::integer, 1, 1)
        + (substring(etiqueta from char_length(cd::text) + 5 for 3)::integer - 1),
    dv = app.aud_caixa_strip_leading_zeros(substring(etiqueta from char_length(cd::text) + 8 for 6)),
    filial = app.aud_caixa_strip_leading_zeros(
        right(etiqueta, char_length(etiqueta) - char_length(cd::text) - 13)
    )::bigint,
    volume = case
        when nullif(regexp_replace(coalesce(id_knapp, ''), '\D+', '', 'g'), '') is null then null
        else app.aud_caixa_strip_leading_zeros(regexp_replace(id_knapp, '\D+', '', 'g'))
    end
where char_length(etiqueta) in (17, 18)
  and cd is not null
  and left(etiqueta, char_length(cd::text)) = cd::text
  and (char_length(etiqueta) - char_length(cd::text) - 13) in (3, 4);
