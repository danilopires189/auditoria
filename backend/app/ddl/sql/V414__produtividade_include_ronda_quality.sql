do $$
begin
    if to_regprocedure('app.produtividade_events_base_legacy_v414(integer,date,date)') is null then
        alter function app.produtividade_events_base(integer, date, date)
        rename to produtividade_events_base_legacy_v414;
    end if;
end;
$$;
...
as $function$
    select
        ...
        b.source_ref,
        b.event_at
    from app.produtividade_events_base_legacy_v414(p_cd, p_dt_ini, p_dt_fim) b
    ...
    with catalog as (
        select * from (
            values
                ...
                (16, 'ronda_quality_auditoria', 'Ronda de Qualidade', 'auditorias')
