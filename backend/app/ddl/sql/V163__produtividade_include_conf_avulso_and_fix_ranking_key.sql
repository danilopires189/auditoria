-- Include Conferência Volume Avulso in Produtividade details/ranking and fix avulso ranking source key.
    ...
    with catalog as (
        select * from (
            values
                ...
                (9, 'zerados_endereco', 'Inventário (Zerados)', 'endereços'),
                (10, 'devolucao_nfd', 'Devolução de Mercadoria', 'nfd'),
                (11, 'prod_blitz_un', 'Produtividade Blitz', 'unidades'),
                (12, 'prod_vol_mes', 'Volume Expedido', 'volume'),
                (13, 'registro_embarque_loja', 'Registro de Embarque', 'lojas')
    ...
    componentes as (
        select
            ...
            round(
                ...
                3
            )::numeric(18,3) as alocacao_pontos,
            round(
                coalesce(
                    (
                        select greatest(0.5, 3.5 - (rd.pos - 1) * 0.5)
                        from ranks_devolucao rd
                        where rd.user_id = u.user_id
                    ),
                    0
                ),
                3
            )::numeric(18,3) as devolucao_pontos,
            round(
                coalesce(
                    (
                        select greatest(0.5, 3.5 - (rt.pos - 1) * 0.5)
                        from ranks_termo rt
                ...
                3
            )::numeric(18,3) as conf_termo_pontos,
            round(
                coalesce(
                    (
                        select greatest(0.5, 3.5 - (ra.pos - 1) * 0.5)
                        from ranks_avulso ra
                        where ra.user_id = u.user_id
                    ),
                    0
                ),
                3
            )::numeric(18,3) as conf_avulso_pontos,
            round(
                coalesce(
                    (
                        select greatest(0.5, 3.5 - (re.pos - 1) * 0.5)
                        from ranks_entrada re
                        where re.user_id = u.user_id
                    ),
                    0
                ),
                3
            )::numeric(18,3) as conf_entrada_pontos,
            round(
                coalesce(
                    (
                        select greatest(0.5, 3.5 - (rl.pos - 1) * 0.5)
                        from ranks_lojas rl
                ...
                3
            )::numeric(18,3) as conf_lojas_pontos,
            round(
                coalesce(
                    (
                        select um.total_val
            ...
            round(
                coalesce(
                    (
                        ...
                        from usuarios_metricas um
                        where um.user_id = u.user_id
                          and um.activity_key = 'avulso_sku'
                    ),
                    0
                ),
                3
