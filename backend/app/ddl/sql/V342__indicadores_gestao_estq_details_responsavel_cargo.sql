create index if not exists idx_app_db_usuario_cd_mat_norm
    on app.db_usuario (cd, authz.normalize_mat(mat))
    include (mat, nome, cargo);

drop function if exists public.rpc_indicadores_gestao_estq_details(integer, date, date, text, integer);
...
begin
    ...
    select
        ...
        a.ocorrencias
    from aggregated a
    order by a.data_mov desc, a.valor_total desc, a.coddv asc, a.responsavel asc
    limit v_limit;
end;
$$;
