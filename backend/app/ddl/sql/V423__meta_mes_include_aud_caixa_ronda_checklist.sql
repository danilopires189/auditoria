alter table app.meta_mes_daily_targets
    drop constraint if exists meta_mes_daily_targets_activity_key_check,
    drop constraint if exists ck_meta_mes_daily_targets_activity_key;

alter table app.meta_mes_daily_targets
    add constraint ck_meta_mes_daily_targets_activity_key
...
alter table app.meta_mes_month_targets
    drop constraint if exists meta_mes_month_targets_activity_key_check,
    drop constraint if exists ck_meta_mes_month_targets_activity_key;

alter table app.meta_mes_month_targets
    add constraint ck_meta_mes_month_targets_activity_key
    check (activity_key in (
        'pvps_coddv',
        'alocacao_coddv',
        'blitz_unidades',
...
as $$
    values
        ...
        (9, 'auditoria_caixa_volume'::text, 'Auditoria de Caixa'::text, 'volumes'::text, 'integer'::text),
        (10, 'checklist_auditoria'::text, 'Check List'::text, 'checklists'::text, 'integer'::text),
        (11, 'ronda_quality_auditoria'::text, 'Ronda de Qualidade'::text, 'auditorias'::text, 'integer'::text);
$$;
...
as $$
    ...
    aud_caixa_daily as (
        select
            ...
            timezone('America/Sao_Paulo', c.data_hr)::date as date_ref,
            count(*)::numeric(18, 3) as actual_value,
            max(c.updated_at) as updated_at
        from app.aud_caixa c
        cross join month_bounds mb
        where c.cd = p_cd
          and timezone('America/Sao_Paulo', c.data_hr)::date >= mb.month_start
          and timezone('America/Sao_Paulo', c.data_hr)::date <= mb.month_end
        group by timezone('America/Sao_Paulo', c.data_hr)::date
    ),
    checklist_daily as (
        select
            'checklist_auditoria'::text as activity_key,
            timezone('America/Sao_Paulo', coalesce(a.signed_at, a.created_at))::date as date_ref,
            count(*)::numeric(18, 3) as actual_value,
            max(a.updated_at) as updated_at
        from app.checklist_dto_pvps_audits a
        cross join month_bounds mb
        where a.cd = p_cd
          and timezone('America/Sao_Paulo', coalesce(a.signed_at, a.created_at))::date >= mb.month_start
          and timezone('America/Sao_Paulo', coalesce(a.signed_at, a.created_at))::date <= mb.month_end
        group by timezone('America/Sao_Paulo', coalesce(a.signed_at, a.created_at))::date
    ),
    ronda_quality_ranked as (
        select
            r.*,
            row_number() over (
                partition by
        ...
        cross join month_bounds mb
        where r.cd = p_cd
          and timezone('America/Sao_Paulo', r.created_at)::date >= mb.month_start
          and timezone('America/Sao_Paulo', r.created_at)::date <= mb.month_end
    ),
    ronda_quality_daily as (
        select
            'ronda_quality_auditoria'::text as activity_key,
            timezone('America/Sao_Paulo', r.created_at)::date as date_ref,
            count(*)::numeric(18, 3) as actual_value,
            max(r.updated_at) as updated_at
        from ronda_quality_ranked r
        where r.row_num = 1
        group by timezone('America/Sao_Paulo', r.created_at)::date
    )
    select
        src.activity_key,
        src.date_ref,
    ...
    from (
        ...
        union all
        select * from aud_caixa_daily
        union all
        select * from checklist_daily
        union all
        select * from ronda_quality_daily
    ) src
