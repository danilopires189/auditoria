-- Devolucao: keep note status/history across days while the note is still in db_devolucao.

create or replace function app.conf_devolucao_upsert_items_from_base(
    p_conf_id uuid,
    p_cd integer,
    p_ref text
)
...
begin
    with scoped as (
        ...
        where d.cd = p_cd
          ...
          and coalesce(nullif(trim(coalesce(d.chave, '')), ''), d.nfd::text) = p_ref
        group by d.coddv
    ),
    barras_best as (
...
begin
    ...
    with notas as (
        select *
        from public.rpc_conf_devolucao_manifest_notas(v_cd)
    ),
    resumo as (
        select
            count(*)::integer as total_etiquetas,
            count(*) filter (where n.status = 'concluido')::integer as conferidas,
            count(*) filter (where n.status = 'em_andamento')::integer as em_andamento,
            bool_or(coalesce(n.tem_falta, false)) as tem_falta
        from notas n
    ),
    em_andamento_actor as (
        ...
        order by n.status_at desc nulls last
        limit 1
    ),
    concluido_actor as (
        select
    ...
    select
        ...
        coalesce(r.total_etiquetas, 0)::integer as total_etiquetas,
        coalesce(r.conferidas, 0)::integer as conferidas,
        greatest(coalesce(r.total_etiquetas, 0) - coalesce(r.conferidas, 0), 0)::integer as pendentes,
        case
            when coalesce(r.em_andamento, 0) > 0 then 'em_andamento'
            when coalesce(r.conferidas, 0) > 0 then 'concluido'
            else 'pendente'
        end as status,
        coalesce(r.tem_falta, false) as tem_falta,
        case
            when coalesce(r.em_andamento, 0) > 0 then ea.colaborador_nome
            when coalesce(r.conferidas, 0) > 0 then ca.colaborador_nome
            else null
        end as colaborador_nome,
        case
            when coalesce(r.em_andamento, 0) > 0 then ea.colaborador_mat
            when coalesce(r.conferidas, 0) > 0 then ca.colaborador_mat
            else null
        end as colaborador_mat,
        case
            when coalesce(r.em_andamento, 0) > 0 then ea.status_at
            when coalesce(r.conferidas, 0) > 0 then ca.status_at
            else null
        end as status_at
    from resumo r
