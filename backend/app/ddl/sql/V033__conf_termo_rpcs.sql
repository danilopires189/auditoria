...
create or replace function public.rpc_conf_termo_manifest_barras_page(
    p_cd integer default null,
    p_offset integer default 0,
    p_limit integer default 1000
)
...
begin
    ...
    conf as (
        select
            c.filial,
            count(distinct c.id_etiqueta)::integer as conferidas
...
create or replace function public.rpc_conf_termo_get_items(p_conf_id uuid)
returns table (
    item_id uuid,
    conf_id uuid,
    coddv integer,
...
create or replace function public.rpc_conf_termo_set_item_qtd(
    p_conf_id uuid,
    p_coddv integer,
...
create or replace function public.rpc_conf_termo_reset_item(
    p_conf_id uuid,
    p_coddv integer
)
returns table (
...
as $$
    select *
    from public.rpc_conf_termo_set_item_qtd(p_conf_id, p_coddv, 0);
$$;
...
grant execute on function public.rpc_conf_termo_manifest_barras_page(integer, integer, integer) to authenticated;
...
grant execute on function public.rpc_conf_termo_get_items(uuid) to authenticated;
grant execute on function public.rpc_conf_termo_scan_barcode(uuid, text, integer) to authenticated;
grant execute on function public.rpc_conf_termo_set_item_qtd(uuid, integer, integer) to authenticated;
grant execute on function public.rpc_conf_termo_reset_item(uuid, integer) to authenticated;
