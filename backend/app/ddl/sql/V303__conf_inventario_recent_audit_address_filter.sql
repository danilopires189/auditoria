alter table app.conf_inventario_admin_seed_config
    add column if not exists ignorar_endereco_auditado boolean,
    add column if not exists auditoria_recente_dias integer;

update app.conf_inventario_admin_seed_config
set
    ignorar_endereco_auditado = coalesce(ignorar_endereco_auditado, false),
    auditoria_recente_dias = greatest(coalesce(auditoria_recente_dias, 0), 0);

alter table app.conf_inventario_admin_seed_config
    alter column ignorar_endereco_auditado set default false,
    alter column ignorar_endereco_auditado set not null,
    alter column auditoria_recente_dias set default 0,
    alter column auditoria_recente_dias set not null;
...
begin
    if not exists (
        select 1
        from pg_constraint
        where conname = 'ck_conf_inventario_admin_seed_config_auditoria_recente_dias'
    ) then
        alter table app.conf_inventario_admin_seed_config
            add constraint ck_conf_inventario_admin_seed_config_auditoria_recente_dias
            check (auditoria_recente_dias >= 0);
    end if;
end;
$$;
...
as $$
    with params as (
        select
            ...
            coalesce(p_ignorar_endereco_auditado, false) as ignorar_endereco_auditado,
            greatest(coalesce(p_auditoria_recente_dias, 0), 0) as auditoria_recente_dias,
    ...
    recent_audited_addresses as (
        select distinct upper(trim(a.endereco)) as endereco
        from (
            select c.endereco, c.cycle_date
            from app.conf_inventario_counts c
            where c.cd = p_cd
            union
            select r.endereco, r.cycle_date
            from app.conf_inventario_reviews r
            where r.cd = p_cd
        ) a
        cross join params p
        where p.ignorar_endereco_auditado
          and p.auditoria_recente_dias > 0
          and a.cycle_date >= (app.conf_inventario_today() - p.auditoria_recente_dias)
          and nullif(trim(coalesce(a.endereco, '')), '') is not null
    ),
    sep_filtered as (
        select
            e.cd,
    ...
    where m.endereco is not null
      ...
      and not exists (
        select 1
        from recent_audited_addresses ra
        where ra.endereco = upper(m.endereco)
      )
...
begin
    ...
    with target as (
        ...
        from app.conf_inventario_seed_target_rows(
            ...
            v_cfg.manual_coddv,
            v_cfg.estoque_tipo,
            v_cfg.ignorar_endereco_auditado,
            v_cfg.auditoria_recente_dias
        )
    )
...
drop function if exists public.rpc_conf_inventario_admin_apply_seed_v2(integer, text[], integer, integer, text, boolean, text, text);
drop function if exists public.rpc_conf_inventario_admin_apply_manual_coddv_v2(integer, text, text, boolean);
drop function if exists public.rpc_conf_inventario_admin_preview_seed(integer, text[], integer, integer, text, boolean, text);
drop function if exists public.rpc_conf_inventario_admin_apply_seed(integer, text[], integer, integer, text, boolean, text, text);
drop function if exists public.rpc_conf_inventario_admin_apply_manual_coddv(integer, text, text, boolean);
...
grant execute on function public.rpc_conf_inventario_admin_preview_seed(integer, text[], integer, integer, text, boolean, integer, boolean, text) to authenticated;
grant execute on function public.rpc_conf_inventario_admin_apply_seed(integer, text[], integer, integer, text, boolean, integer, boolean, text, text) to authenticated;
