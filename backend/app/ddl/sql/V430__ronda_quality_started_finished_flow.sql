alter table app.aud_ronda_quality_sessions
...
begin
    ...
    session_stats as (
        select
            ...
            max(coalesce(s.finished_at, s.created_at)) as last_audit_at,
            (array_agg(s.started_at order by coalesce(s.finished_at, s.created_at) desc, s.started_at desc, s.audit_id desc))[1] as last_started_at,
            (array_agg(coalesce(s.finished_at, s.created_at) order by coalesce(s.finished_at, s.created_at) desc, s.started_at desc, s.audit_id desc))[1] as last_finished_at
...
drop function if exists public.rpc_ronda_quality_zone_detail(integer, text, text, date);
            ...
            coalesce(
                jsonb_agg(
                    jsonb_build_object(
                        ...
                        'finished_at', coalesce(s.finished_at, s.created_at),
                        'occurrence_count', jsonb_array_length(coalesce(op.occurrences, '[]'::jsonb)),
                        'occurrences', coalesce(op.occurrences, '[]'::jsonb)
                    )
                    order by coalesce(s.finished_at, s.created_at) asc, s.started_at asc, s.audit_id asc
                ),
                '[]'::jsonb
            ) as rows
        from app.aud_ronda_quality_sessions s
            ...
            (
                select jsonb_agg(
                    jsonb_build_object(
                        ...
                        'last_started_at', csr.last_started_at,
                        'last_finished_at', csr.last_finished_at
                    )
                    order by csr.coluna
                )
                from column_stats_rows csr
...
drop function if exists public.rpc_ronda_quality_submit_audit(integer, text, text, integer, text, jsonb);
