-- Otimiza rpc_ctrl_validade_linha_retirada_list restringindo os CTEs pesados
-- (ultima_coleta, ultima_retirada, editable_retirada) ao intervalo de meses relevantes,
-- evitando varreduras históricas ilimitadas que causavam timeout conforme o banco crescia.
...
begin
    ...
    with base as (
        ...
        where c.cd = v_cd
          -- Limita ao mês anterior e atual (únicos relevantes para eligibilidade)
          and app.ctrl_validade_month_ref(c.data_coleta) >= v_prev_month_ref
        group by
            c.cd,
            c.coddv,
    ...
    ultima_coleta as (
        ...
        from (
            ...
            from app.ctrl_validade_linha_coletas c
            where c.cd = v_cd
              -- Mesmo filtro de data: só mês anterior e atual
              and app.ctrl_validade_month_ref(c.data_coleta) >= v_prev_month_ref
        ) x
        where x.rn = 1
    ),
    ...
    retirada as (
        ...
        where r.cd = v_cd
          and r.ref_coleta_mes >= v_prev_month_ref
        group by
            r.cd,
            r.coddv,
