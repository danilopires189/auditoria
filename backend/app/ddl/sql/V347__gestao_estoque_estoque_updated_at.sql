alter table app.gestao_estoque_items
    add column if not exists estoque_updated_at timestamptz;
...
update app.gestao_estoque_items i
   set estoque_updated_at = r.estoque_updated_at
  from resolved r
 where i.id = r.id
   and i.estoque_updated_at is distinct from r.estoque_updated_at;
