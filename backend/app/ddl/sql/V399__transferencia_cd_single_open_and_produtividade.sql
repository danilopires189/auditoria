...
declare
    ...
    v_etapa text;
    v_conf app.conf_transferencia_cd%rowtype;
    v_user_active app.conf_transferencia_cd%rowtype;
    v_profile record;
    v_cd_ori_nome text;
    v_cd_des_nome text;
begin
    ...
    if v_user_active.conf_id is not null
       and (
          v_user_active.dt_nf <> p_dt_nf
          or v_user_active.nf_trf <> p_nf_trf
          or v_user_active.sq_nf <> p_sq_nf
          or v_user_active.cd_ori <> p_cd_ori
          or v_user_active.cd_des <> p_cd_des
          or v_user_active.etapa <> v_etapa
       ) then
        raise exception 'CONFERENCIA_EM_ABERTO_OUTRA_TRANSFERENCIA';
    end if;
